...
func (o *UpdateScheduleURL) Build() (*url.URL, error) {
	...
	var _path = "/v{eventId}/schedule"

	eventID := swag.FormatInt64(o.EventID)
	if eventID != "" {
		_path = strings.Replace(_path, "{eventId}", eventID, -1)
	} else {
		return nil, errors.New("eventId is required on UpdateScheduleURL")
	}
...
func (o *UpdateScheduleURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateScheduleURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateScheduleURL")
	}
